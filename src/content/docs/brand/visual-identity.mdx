...
<Tabs>
  ...
  <TabItem label="Accessibility">
    ...
    3:1 for large text). - For example, "Nordic Blue" (#0053A5) on "White Snow"
    (#FFFFFF) has a contrast ratio of 8.59:1, which is excellent and meets AA
    and AAA guidelines. - Conversely, "Lake Blue" (#76B7ED) on "White Snow"
    (#FFFFFF) has a contrast ratio of 2.95:1. This combination is not acceptable
    for normal-sized body text as it fails the AA standard, but it could be
    acceptable for large text (e.g., headings). - Never use color as the only
    indicator of meaning. - Provide sufficient contrast between text and
    backgrounds. - Test color combinations with accessibility tools to ensure
    compliance.
  </TabItem>
</Tabs>
